// Central place for the site's external links + contact info.
// Editing one value here updates every button/link across the site.
export const SITE_URL = "https://daniel-rosen.vercel.app";

export const links = {
  // NOTE: verify this LinkedIn URL while logged in (the trailing hyphen is unusual).
  // Whatever the correct handle is, change it once here and it updates everywhere.
  linkedin: "https://www.linkedin.com/in/daniel-rosen-",
  resume: "/resume.pdf",
  email: "danirosen18@gmail.com",
  phone: "+17867071255",
  phoneDisplay: "(786) 707-1255",
} as const;
